// Command SvobodaWeb-cli is a CLI + REPL for the SvobodaWeb SDK,
...
const prompt = "svoboda-web"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "highlight"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SVOBODA_WEB_APIKEY (api key), SVOBODA_WEB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SVOBODA_WEB_APIKEY carries the API key and
	// SVOBODA_WEB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SVOBODA_WEB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SVOBODA_WEB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSvobodaWebSDK(opts)
