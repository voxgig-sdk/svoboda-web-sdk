...
boru-driven command-line client **and** interactive REPL for the SvobodaWeb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/svoboda-web-cli)
make build

# 2. See usage (words, entities, env vars)
./svoboda-web-cli --help

# 3. Provide credentials once, via the environment
export SVOBODA_WEB_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./svoboda-web-cli list highlight

# 5. Override the API base URL for a single call
SVOBODA_WEB_BASE=https://api.example.com ./svoboda-web-cli list highlight

# 6. No arguments -> interactive REPL
./svoboda-web-cli
svoboda-web> list highlight
svoboda-web> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/svoboda-web-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/svoboda-web-cli list highlight
   ```
...
Run with no arguments to open a REPL (prompt `svoboda-web>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `highlight`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/svoboda-web-cli`. |
...
(`./svoboda-web-cli <expr>`) and interactively (the REPL), and expressions compose the
