...
class FtCtrl extends SvobodaWebControl
{
...
class FtClient
{
    ...
    private SvobodaWebUtility $utility;

    public function __construct(SvobodaWebUtility $utility, array $options)
    {
    ...
    public function get_utility(): SvobodaWebUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SvobodaWebContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SvobodaWebContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SvobodaWebContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SvobodaWebConfig::make_feature($name);
    ...
    private static function buildUrl(SvobodaWebSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SvobodaWebError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SvobodaWebResponse($fetched);
        }
    ...
    private function fail(SvobodaWebContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SvobodaWebContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
