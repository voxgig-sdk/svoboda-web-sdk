...
class HighlightEntity
{
    ...
    {
        if ($args) {
            $this->_data = SvobodaWebHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = SvobodaWebHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws SvobodaWebError on failure (item-5 convention).
     */
