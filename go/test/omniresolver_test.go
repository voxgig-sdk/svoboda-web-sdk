...
import (
	...
	omni "github.com/voxgig-sdk/svoboda-web-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SvobodaWebError); is {
				out := map[string]any{
					"name":    "SvobodaWebError",
...
func omniCtx(arg any, client *sdk.SvobodaWebSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SvobodaWebSDK); is {
			client = live
