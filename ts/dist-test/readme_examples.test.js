...
const TEST_SEED = { "entity": { "highlight": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+SvobodaWebSDK\b/.test(code) ||
        /\bSvobodaWebSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+SvobodaWebSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/SvobodaWebSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
