...
require_relative 'SvobodaWeb_types'
...
class SvobodaWebSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SvobodaWebHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SvobodaWebHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SvobodaWebFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SvobodaWebUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SvobodaWebHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SvobodaWebHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SvobodaWebHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SvobodaWebError.new(
        "#{op}_allow",
        "SvobodaWebSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SvobodaWebHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SvobodaWebError.new(
        "graphql_error", "SvobodaWebSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
