...
module SvobodaWebUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SvobodaWebResponse.new({})
    ...
    if fetch_err
      ...
      response = SvobodaWebResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SvobodaWebResponse.new(fetched)
    else
