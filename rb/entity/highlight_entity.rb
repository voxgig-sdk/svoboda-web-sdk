# SvobodaWeb SDK Highlight entity
...
class HighlightEntity
  ...
  def data_set(args)
    if args
      @_data = SvobodaWebHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SvobodaWebHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Highlight>, Array] the matching Highlight items; raises SvobodaWebError on failure
