...
[MCP](https://modelcontextprotocol.io) server exposing the SvobodaWeb SDK as
two agent tools — `svoboda-web_list` and `svoboda-web_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/svoboda-web-mcp)
make build

# 2. Provide credentials via the environment
export SVOBODA_WEB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user svoboda-web \
  -- /absolute/path/to/svoboda-web-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./svoboda-web-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// svoboda-web_list: first page of records
{ "entity": "highlight" }
{ "entity": "highlight", "query": { } }

// svoboda-web_load: one record by id
{ "entity": "highlight", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/svoboda-web-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user svoboda-web \
     -- "$PWD"/dist/*/svoboda-web-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `svoboda-web_list` and `svoboda-web_load` tools now appear
   in new sessions. Ask the agent to *"list highlight using svoboda-web"*
   and it calls `svoboda-web_list` with `{"entity":"highlight"}`.
...
```sh
export SVOBODA_WEB_APIKEY=sk_live_xxx            # API key
export SVOBODA_WEB_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `svoboda-web_list` tool
...
### Call the `svoboda-web_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `svoboda-web_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `svoboda-web_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SVOBODA_WEB_APIKEY` | API key sent with every request. |
| `SVOBODA_WEB_BASE` | Optional override of the API base URL. |
...
```sh
./svoboda-web-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"svoboda-web_load","arguments":{"entity":"highlight","query":{"id":1}}}}'
```
