...
class SvobodaWebTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SvobodaWebTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SvobodaWebTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SvobodaWebTestRunner.getenv("SVOBODA_WEB_TEST_LIVE")
        override = SvobodaWebTestRunner.getenv("SVOBODA_WEB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SvobodaWebTestRunner.getenv(key)
        ...
        explain = SvobodaWebTestRunner.getenv("SVOBODA_WEB_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SvobodaWebTestRunner._test_control is not None:
            return SvobodaWebTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SvobodaWebTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SvobodaWebTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SvobodaWebTestRunner.load_test_control()
...
def load_env_local():
    SvobodaWebTestRunner.load_env_local()


def env_override(m):
    return SvobodaWebTestRunner.env_override(m)


def entity_data(v):
    return SvobodaWebTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SvobodaWebTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SvobodaWebTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SvobodaWebTestRunner.load_test_control()


def live_client_options():
    return SvobodaWebTestRunner.live_client_options()


def live_delay_ms():
    return SvobodaWebTestRunner.live_delay_ms()
